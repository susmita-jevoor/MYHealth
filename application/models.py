...
def upload_report(username, report_file, filename):
    ...
    report_id = reports_collection.insert_one({
        ...
        'uploader_role': 'patient',  # Marking the uploader as 'doctor'
        'upload_date': datetime.combine(datetime.today().date(), datetime.min.time()) 
    }).inserted_id
    ...
    try:
        chat_completion = client_groq.chat.completions.create(
            messages=[
                {
                    "role": "user",
                    "content": f"Make the summary more precise into different points numbered in different lines: {text}",
                }
            ],
            model=model,
        )
...
def create_prescription(username, hospital_name, medication_name, dosage, before_food, after_food, morning, afternoon, evening, remarks, note):
    ...
    prescription = {
        ...
        'evening': evening,
        'remarks': remarks,
        'note': note  # Optional: add a timestamp for when the prescription was added
    }
...
def save_contact(name, email, message):
    ...
    contact_data = {
        ...
        'email': email,
        'message': message,
        'submitted_at': datetime.combine(datetime.today().date(), datetime.min.time()) 
    }
